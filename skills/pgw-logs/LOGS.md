...
- Logs are one JSON object per line, filtered through a **typed allowlist** in
  ...
  a bug: report it.
- **There is no field a user-defined secret term can travel in.** `mask_terms` values are
  enterprise secrets by construction — that is why someone named one — so the allowlist
  carries only `term_count` and `surviving_term_count`. A term appearing in a log line
  would be the leak the feature exists to prevent; report it.
...
| `mask.gemma.cached`               | gateway   | the chunk was extracted earlier in this process and answered from memory; `text_length` only. On an agent-CLI turn most chunks should hit this — that is what keeps a CLI-sized prompt (~58 KiB forwarded, measured) inside its deadline. All misses on a repeat turn ⇒ a new instance, or an unstable preamble |
...
judge is capped at **1024**, which is ample for a verdict object and much tighter than
the extractor needs. A bounded generation cannot pin the GPU indefinitely, which is what
turned an ordinary slow request into a fleet-wide outage. The cap in turn is what makes
**chunked extraction** necessary — a large prompt's span list does not fit in 4096
tokens, so the input is split (`EXTRACTION_CHUNK_BYTES`, code default 12000, deployed 4000) and extracted **4 chunks at a time** (`EXTRACTION_CONCURRENCY`, default 4), using
all four of Gemma's llama.cpp slots. **No slot is reserved for the judge**: it runs after
masking, not beside it, so the reserved slot was idle during the only phase that could
have used it.

That concurrency is a **global** cap, held by one semaphore for the whole extraction. It
bounds actual Gemma calls across chunking _and_ every level of the bisection recursion —
not each level separately, which is what let four simultaneously-failing chunks open 8
concurrent calls and then 16. The request's deadline signal reaches the same semaphore,
so after a 504 no further chunk is dequeued.
