...
| シグナル     | 得られるもの                                                                                                                                                                                                                                                                               |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `request_id` | Gateway が**リクエストごとに発行する** UUIDv7。vault のキーとして使われる。受信した `X-Request-ID` ヘッダは完全に無視される——詳細は下の[セッションの廃止](#セッションの廃止)。Gateway → Core → Synthesis に伝播し、`X-Request-ID` レスポンスヘッダで返され、OKF frontmatter にも保存される |
...
`POST /v1/ask` が受け取るのは `{text, rehydrate_allow?, mask_terms?}` だけで、`session_id`
やその他の呼び出し側が指定する ID、未知のフィールドを含むボディはスキーマの `strict()` に
よって `400` で拒否される。Gateway はリクエストごとに 1 つのサーバ生成 request id
（UUIDv7）を発行し、それを Token Vault のキーとして使う。受信した `X-Request-ID` ヘッダは
完全に無視され、`X-Request-ID` レスポンスヘッダが返すのは常に Gateway が発行した ID で、
呼び出し側が送った値ではない。
...
`just check` は CI 相当のチェック — lint・フォーマット・Terraform 検証・typecheck・pin
検証・シークレットスキャン・ユニットテスト — をローカルで 1 コマンドにまとめたものである。
ブラウザ E2E は別ゲートの `just web-e2e`。CI は同じ範囲をカバーするが、`just check` を呼ぶ
のではなく複数ジョブに分割して並列実行し、両方のゲートを `main` への push と全ての pull
request で走らせる。何が通っているか、そしてある時点のコミットでテストが何件あるかの
