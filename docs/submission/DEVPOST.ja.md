...
- **ユーザー定義の秘匿語句**: 未発表の製品名や社内コードネームには検出できる形がないため、依頼者が `mask_terms` で指定すると全検出器より前に ⟦CUSTOM_1⟧ へ置換される。境界の両スキャンがリテラル文字列を探すので、マスキングが機能したことを*証明*できる唯一のチェックになる（他は判定に使ったパターンの再実行にすぎない）。語句は永続化せず、監査記録は件数のみを残す
