...
同じ理屈から、配信は **終端的 (terminal)** である。開示ゲートがリトライするのは、拒否の
取りこぼしがデータ漏洩になるからだ。コストゲートが永遠にリトライすれば、それ自体が防ぐはず
だった障害になる。実発火がそれを証明した（[proof/kill-switch.md](proof/kill-switch.md)）:
`scaleToZero` の失敗が 500 を返し、Pub/Sub は 11 分間 30 秒おきに再配信して、運用者が復旧する
...
**デッドレター。** `billing-kill-switch-dead-letter`（`max_delivery_attempts = 5`）が、
ハンドラ側では応答しようのないトランスポート層のリトライループを打ち切る（起動しないサービス、
...
- **GPU は撮影前に `just warm` で暖めておく**（§9.4 参照）。約 90 秒のコールドスタートを
  待つ画は動画が持たない
...
撮影セッションは次の 2 つのレシピで挟む。

```bash
just warm    # インスタンスを 1 つ常駐: gemma-serving min-instances=1
# ... 撮影・デモ・審査員の操作 ...
just chill   # 解放: min-instances=0、課金停止
```

この間は `just smoke` も UI も、1 分半ではなく数秒で応答する。
...
`just warm` の後、録画前に `just smoke` を **1 回** 投げること。`min-instances=1` は
コンテナを起動するだけで、Ollama がモデルを読み込むのは最初のリクエスト到達時なので、
一度も叩いていない warm インスタンスは本番でモデルロード時間を払うことになる。

Terraform ではなく `gcloud` レシピにしている理由: 暖めた GPU は一時的な運用状態であって
desired state ではない。Terraform に書くとリポジトリの構成が「GPU は常時稼働すべき」と
...
`GET /audit` は、OKF evidence ストアを審査員がブラウズできる形にした読み取り専用ページで
ある。保存済みの `Gateway Answer` ドキュメント 1 件につき 1 行を新しい順に並べ、status、
導出された trust tier、attestation の verdict、マスクしたカテゴリ数、judge のリトライ回数、
withheld カテゴリを表示する。行をクリックすると `/v1/requests/<id>` を取得し、frontmatter
はラベル付きの事実として、本文はテキストとして描画したうえで、そこに記載された 2 つのマスク
済みソースと Cloud Trace へのリンクを添える。

**表示されるものはすべてマスク済みの evidence である。** 再水和済みの回答は 1 回の API
レスポンスで返すだけでどこにも保存しないので、そもそもこのページが表示できる非マスクのデータ
は原理的に存在しない。ページ自身のヘッダーにもそう明記してある。また厳密な意味で読み取り
専用であり、承認・削除・再実行のいずれも存在しない。報告内容を自分で変更できる監査ビューは
evidence ではないからである。

#### トークンは identity ではなく capability

アクセスは共有文字列 1 本の `ADMIN_TOKEN` でゲートする。
...
`human:` actor を作ることは決してない**。身元不明のトークン保持者がクリックした承認は「誰
でもない人」の主張になるからで、これがこのプロダクトで human review をスコープ外にしている
理由そのものである（`docs/ARCHITECTURE.md` 参照）。このビューが並べるドキュメントは、最良で
も `machine-confirmed` に留まる。

またトークンが買えるのは*列挙*だけである。`/v1/requests/<id>` は元々 capability によって公開
されており（UUIDv7 を知っていること自体が capability）、監査ビューが追加するのは「何が存在
するかを一覧する」能力であって、「本来読めないものを読む」能力ではない。

#### 有効化・ローテーション・無効化

トークンは Terraform 変数（`admin_token`、`sensitive`、既定値 `""`）である。コミットは決して
せず、apply ごとに渡す。
