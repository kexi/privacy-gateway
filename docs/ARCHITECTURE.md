...
| -------------------------- | ---------------------------------------------- | -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MAX_BODY_BYTES` (gateway) | 64 KiB                                         | 512 KiB                                | Codex CLI turns carry an instruction block far past 64 KiB — with a loaded skill catalogue, past 256 KiB. Cost stays bounded by the rate limit, the deadline, one GPU instance and the kill switch.                                                                                                                                                  |
| `SYNTHESIS_MAX_BODY_BYTES` | `MAX_BODY_BYTES × 2 + 64 KiB` (192 KiB)        | 1088 KiB                               | Synthesis receives a whole masked prompt **plus** a whole Core answer in one body, so its limit must be derived from the gateway's, never copied.                                                                                                                                                                                                    |
| `REQUEST_DEADLINE_SECONDS` | 60 s                                           | 240 s                                  | 60 s is right once Gemma is warm. A scale-from-zero request also waits for Ollama to load `gemma4:12b` (~8 GB; worst case from cold ~90 s), and a measured Codex-scale turn masks 16 chunks. With the schema-constrained, no-thinking extractor a real warm Codex CLI turn completes in ~30 s (measured 2026-08-31); 240 s covers the cold case too. |
