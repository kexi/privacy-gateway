...
| ------------------- | ---------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Gateway Agent**   | Gemma 4 (self-hosted)  | Cloud Run     | ADK TypeScript. Receives the user request (`{text}` only) and mints one server-generated `request_id` (UUIDv7) that also serves as the Token Vault key. Detects PII/secrets (hybrid: deterministic regex + Gemma span extraction), replaces with stable tokens `⟦PERSON_1⟧`, `⟦EMAIL_1⟧`, `⟦SECRET_1⟧`. Stores the mapping in the **Token Vault** (Firestore, per-request, TTL). Forwards the _masked_ prompt to Core via A2A; reaches Synthesis over plain HTTP. Serves the demo UI and the HTTP API from one origin.                                                                                                                                                                                                                            |
