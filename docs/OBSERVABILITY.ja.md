...
キーとして使う。受信した `X-Request-ID` ヘッダは**完全に無視される** — 受信値が
エコーされることはなく、`X-Request-ID` レスポンスヘッダが返すのは常に Gateway が
発行した ID である。そのリクエスト自身の ID として**採用されることは決してない**:
...
| `number`       | 有限の数値                                                             | `duration_ms`, `attempt`, `port`, `placeholder_count`, `masked_count`, `unstructured_spans`, `span_count`, `chunk_count`, `depth`, `tokens_resolved`, `tokens_unknown`, `withheld_count`, `vault_generation`, `body_bytes`, `finding_count`, `text_length`, `tokens_withheld`, `term_count`, `surviving_term_count` |
...
| `mask.gemma.chunked`     | INFO          | 入力が `EXTRACTION_CHUNK_BYTES` を超えたため分割して抽出した。`chunk_count`, `duration_ms`, `text_length`。この行が無ければ単一呼び出し経路。`chunk_count` が `text_length / 12000` を大きく上回る場合は境界探索が早く切りすぎている                                                                 |
| `mask.gemma.bisected`    | INFO          | リトライ後も読めなかったチャンクを半分に割って再抽出した。`depth`, `chunk_count`, `text_length`。PII 密集入力では想定内。毎回 `depth` が下限まで達する場合は予算不足ではなくモデルが契約に従っていない                                                                                               |
| `mask.gemma.cached`      | INFO          | 同一プロセス内で抽出済みのチャンクをメモリから返した。載るのは `text_length` のみ。エージェント CLI からのリクエストで多数出るのは意図どおり（静的な指示プリアンブルの代金は 1 回だけ）。2 ターン目以降で 1 件も出ないならインスタンスが入れ替わったか、呼び出し側のプリアンブルが実は安定していない |
| `mask.gemma.unparseable` | INFO/WARNING  | Gemma のスパン JSON を利用できず 1 回リトライ。恒常的なら JSON モードかモデル未取得。チャンク分割経路ではチャンク単位で発生し、リトライ後も読めないチャンクが 1 つでもあれば抽出全体が失敗する                                                                                                       |
...
| `openai.compat.responses.start`    | INFO     | Responses API のリクエストを受理。`ok` は stream 要求の有無（Codex は常に要求する）。入力本文は決して出力しない                                                                      |
| `openai.compat.responses.end`      | INFO     | Responses オブジェクトを整形した。`document_status`, `trust_tier`                                                                                                                    |
| `openai.compat.responses.refused`  | ERROR    | ゲートが拒否した。`error_code`, `categories`。ストリーミング中の呼び出し側には終端イベント `response.failed` として届く（200 が既に送出済みのため）。完了ターンになることはない      |
| `openai.compat.responses.rejected` | WARNING  | パイプライン実行前にボディ検証で失敗。`error_code` の集合は chat 面と同じ                                                                                                            |
