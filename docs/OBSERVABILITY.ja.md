...
| `number`       | 有限の数値                                                             | `duration_ms`, `attempt`, `port`, `placeholder_count`, `masked_count`, `unstructured_spans`, `span_count`, `tokens_resolved`, `tokens_unknown`, `withheld_count`, `vault_generation`, `body_bytes`, `finding_count`, `text_length`, `tokens_withheld` |
...
| `mask.done`              | INFO          | `placeholder_count`, `counts_by_category`, `unstructured_spans`, `vault_generation`。PII を含む入力で 0 なら検出器の退行                                                                               |
...
| `judge.retry`                                      | INFO         | ジャッジを 1 度だけ再実行した。決定的 attester が合格済みのボディに対し、根拠カテゴリを挙げずに leak を主張した場合に限る。`verdict` は 2 回目の答え（`flagged` / `clear` / `unusable`）。根拠カテゴリ付きの flag は再実行しない |
| `release.refused`                                  | ERROR        | いずれかのゲートがリリースを拒否した。`refusal` は `vault_missing`、`vault_expired`、`vault_generation_mismatch`、`invented_token`、`leak_check_failed`、`judge_flagged`、`judge_unavailable`、`unresolved_token` のいずれか     |
| `release.ok`                                       | INFO         | すべてのゲートを通過してリリースされた。`tokens_resolved`, `withheld_count`                                                                                                                                                      |
