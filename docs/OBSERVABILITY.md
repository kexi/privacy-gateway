...
| `number`      | a finite number                                              | `duration_ms`, `attempt`, `port`, `placeholder_count`, `masked_count`, `unstructured_spans`, `span_count`, `chunk_count`, `depth`, `tokens_resolved`, `tokens_unknown`, `withheld_count`, `vault_generation`, `body_bytes`, `finding_count`, `text_length`, `tokens_withheld`, `term_count`, `surviving_term_count` |
...
| `mask.gemma.chunked`     | INFO          | the input exceeded `EXTRACTION_CHUNK_BYTES` and was extracted in pieces; `chunk_count`, `duration_ms`, `text_length`. Absent ⇒ the request took the single-call path. A `chunk_count` far above `text_length / 12000` means the boundary search is splitting early                                                           |
| `mask.gemma.bisected`    | INFO          | a chunk was still unreadable after its retry and was halved and re-extracted; `depth`, `chunk_count`, `text_length`. Expected on PII-dense input. `depth` climbing to the floor on every request ⇒ the model is not complying, not merely over budget                                                                        |
| `mask.gemma.cached`      | INFO          | this chunk was extracted earlier in this process and answered from memory; `text_length` only. Many of these on an agent-CLI request is the intended behaviour — the static instruction preamble is paid for once. None at all on a repeat turn ⇒ the instance was replaced, or the caller's preamble is not actually stable |
| `mask.gemma.unparseable` | INFO/WARNING  | Gemma's span JSON could not be used; retried once. Persistent ⇒ JSON mode or model not pulled. On the chunked path this is per chunk, and one chunk still unusable after its retry fails the whole extraction                                                                                                                |
...
| `openai.compat.responses.start`    | INFO     | a Responses-API request was accepted; `ok` carries whether the caller asked to stream (Codex always does). Never the input text                                                     |
| `openai.compat.responses.end`      | INFO     | the Responses object was shaped; `document_status`, `trust_tier`                                                                                                                    |
| `openai.compat.responses.refused`  | ERROR    | a gate refused; `error_code`, `categories`. A streaming caller receives this as a terminal `response.failed` event, because the 200 is already on the wire — never a completed turn |
| `openai.compat.responses.rejected` | WARNING  | the body failed validation before the pipeline ran; same `error_code` set as the chat surface                                                                                       |
...
| `release.refused`                                  | ERROR        | a gate refused the release; `refusal` is one of `vault_missing`, `vault_expired`, `vault_generation_mismatch`, `invented_token`, `leak_check_failed`, `judge_flagged`, `judge_unavailable`, `unresolved_token`, `rehydration_incomplete`. On `rehydration_incomplete` the line also carries `error_code` naming which check broke (`leftover_token` / `missing_withheld` / `substitution_mismatch` from the diagnostic preambles, or `rebuild_mismatch` from the decisive positional rebuild) plus the offending token **names** — never a value, and no categories. `rebuild_mismatch` carries an **empty** token list: the difference is between two whole strings, neither of which may be quoted |
