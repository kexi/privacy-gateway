...
| `judge.retry`                                      | INFO         | the judge was re-asked once, because it flagged a leak while naming no category over a body the deterministic attester had already passed. `verdict` is the second answer (`flagged` / `clear` / `unusable`). A flag that names categories is never retried |
