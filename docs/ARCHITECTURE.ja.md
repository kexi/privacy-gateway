...
1. **残存集合が抑制集合と一致する。** ポリシーが抑制していない `⟦…⟧` が残っていれば、
   置換が黙って失敗したということ。逆に抑制対象のプレースホルダが*消えて*いれば、何かが
   それを置換したということであり、それはポリシーが禁じた開示そのものである。
2. **復元された各プレースホルダが金庫の値そのものを持つ。** 別の値を書いたリハイドレータ
   でも、プレースホルダのない文字列を作れば (1) は通ってしまう。
3. **それ以外に PII らしきものが出現していない。** 意図的に復元した値を差し引いた上で、
   リリーステキストに対して決定的アテスタを再実行し、残差が空であることを要求する。
   素のまま走査すれば成功したリリースはすべてフラグされる — 成功とは実識別子が入って
   いることだからだ。
