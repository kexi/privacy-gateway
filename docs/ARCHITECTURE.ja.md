...
`REQUEST_DEADLINE_SECONDS` を 150 秒より延ばす案も同じ理由で見送った: キャパシティ限界を
取り除くのではなく覆い隠すだけであり、実際に効くのは 2 ターン目以降を安くするチャンク
キャッシュの方だからである。
...
| `REQUEST_DEADLINE_SECONDS`  | 60 秒                                           | 150 秒                                    | Gemma が温まっていれば 60 秒で足りる。ゼロからのスケール時は Ollama の `gemma4:12b`（約 8 GB）ロード待ちが加わり、実測の最悪値は約 90 秒。       |
...
コスト事故の火種になる。抽出の並列度は**グローバル**な上限である — チャンク分割と
bisection 再帰のすべての階層で 1 つの semaphore を共有しており、階層ごとの幅ではない。
そのため、チャンクが同時に全滅しても自分自身のファンアウトを増やすことはできない。
