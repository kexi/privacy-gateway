...
依然として収まらない。さらに延ばす案は見送った: それはキャパシティ限界を取り除く
...
| `REQUEST_DEADLINE_SECONDS`  | 60 秒                                           | 240 秒                                    | Gemma が温まっていれば 60 秒で足りる。ゼロからのスケール時は Ollama の `gemma4:12b`（約 8 GB、実測の最悪値は約 90 秒）ロード待ちが加わり、実測の Codex 規模ターンは 15 チャンクをマスクする。240 秒はそれをカバーするが、コールドの完全な Codex CLI 経路は依然収まらない（§9a）。 |
