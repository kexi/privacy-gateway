...
- **0:25–1:10** PII 入りテキストを送信 → マスク済みプロンプト（⟦PERSON_1⟧ 等）を見せる
  - **任意ビート（約15秒）**: 「Additionally mask these phrases」に `Titan Project` を入れて
    2 本目を送る。「Titan Project が機密だと知っている正規表現は存在しない — それは文字列の
    性質ではなく会社側の事実だから。だから依頼者が指定し、⟦CUSTOM_1⟧ として境界を越える。
    ガードは送出プロンプトからその文字列そのものを探す — 判定に使ったパターンを再実行するの
    ではなく、マスキングが機能したことを証明できる唯一のチェックだ。名前は手元に戻るが、
    Gemini は一度も見ていない」。尺が押していれば省略してよい（本筋は成立する）
